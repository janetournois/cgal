...
public:
  ...
  Result_type collapse()
  {
    ...
    for (typename Tr::Finite_cells_iterator cit = triangulation.finite_cells_begin();
         ...
         ++cit)
    {
      ...
      for (typename Tr::Finite_vertices_iterator vit = triangulation.finite_vertices_begin();
                          ...
                          Visitor& visitor)
{
  ...
  for (typename T3::Finite_edges_iterator eit = tr.finite_edges_begin();
       eit != tr.finite_edges_end();
       ++eit)
  {
    const typename T3::Edge e = *eit;
